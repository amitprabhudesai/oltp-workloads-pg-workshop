...
\begin{frame}{The Durability Problem}
  ...
  \begin{block}{}
    ...
    in shared buffers --- in RAM.\\[0.4em]
...
% -----------------------------------------------------------------------------
\begin{frame}[fragile]{Tuple Insertion with WAL}

  \begin{columns}[T]
    \column{0.44\textwidth}
    \begin{enumerate}\small
      \item \textbf{Checkpoint}: checkpointer writes a checkpoint record
            containing the latest \textbf{REDO point} to WAL.
...
\begin{lstlisting}[style=ascii]
  Shared Buffers          WAL Buffer / pg_wal/
  +----------------+      +--------------------+
  | TABLE_A        |      |                    |
  | [tup_1, tup_2] | <==  | XLOG @ LSN_2       |
  | pd_lsn=LSN_2   |      | XLOG @ LSN_1       |
  +----------------+      | CKPT @ REDO point  |
                          +--------------------+
          CRASH                    |
            |                      v  (survives)
  ...
  Recovery: read WAL from REDO point,
            replay each XLOG record in order
\end{lstlisting}
  \end{columns}
\end{frame}
...
\begin{frame}[fragile]{Database Recovery Using WAL}
  ...
  \begin{columns}[T]
    \column{0.44\textwidth}
    PostgreSQL replays XLOG records sequentially from the
    \textbf{REDO point} (the LSN recorded at the last checkpoint).

    \vspace{0.4em}
    \textbf{Replay rule:} compare the XLOG record's LSN with
    the page's \texttt{pd\_lsn}:
    \begin{itemize}\small
      \item \texttt{LSN\_record} $>$ \texttt{pd\_lsn}: apply the change
            and update \texttt{pd\_lsn}
      \item \texttt{LSN\_record} $\leq$ \texttt{pd\_lsn}: skip --- already
            on disk before the crash
    \end{itemize}

    \vspace{0.3em}
    {\small This idempotent check makes recovery safe whether or not
    dirty pages reached disk before the crash.}

    \column{0.54\textwidth}
\begin{lstlisting}[style=ascii]
  pg_wal/ (survived crash)
  +-----------------------------------+
  | CKPT record  --> REDO point       |
  | XLOG INSERT  --> LSN_1            |
  | XLOG INSERT  --> LSN_2            |
  +-----------------------------------+
          | replay from REDO point
          v
  1. Load TABLE_A page from storage
     (pd_lsn = LSN_0, stale on disk)
  2. LSN_1 > LSN_0: apply insert
     pd_lsn = LSN_1
  3. LSN_2 > LSN_1: apply insert
     pd_lsn = LSN_2
...
\begin{frame}[fragile]{Checkpoints: Bounding Recovery Time}
  \begin{columns}[T]
    \column{0.54\textwidth}
    \textbf{What a checkpoint does:}
    \begin{enumerate}\small
      \item Flush all dirty pages from shared buffers to data files
      \item Write a checkpoint WAL record with the \texttt{redo\_lsn}
    \end{enumerate}
    \textbf{On crash:} replay only WAL \textit{after} \texttt{redo\_lsn}.

    \vspace{0.4em}
    \textbf{Triggered by:}
    \begin{itemize}\small
      \item \texttt{checkpoint\_timeout} (default 5\,min)
      \item \texttt{max\_wal\_size} exceeded
      \item Explicit \texttt{CHECKPOINT} command
    \end{itemize}

    \vspace{0.3em}
    {\small Larger \texttt{max\_wal\_size} \textrightarrow\
    more WAL between checkpoints \textrightarrow\
    better write throughput, \textit{but} longer crash recovery ---
    a fundamental production trade-off.}

    \column{0.44\textwidth}
\begin{lstlisting}[style=ascii]
  redo_lsn          current LSN
     |                    |
     v                    v
-----+--------------------+--> WAL
     |<-- replay on crash-|
...
dirty pages already written
to data files at checkpoint
\end{lstlisting}
  \end{columns}
\end{frame}
...
SELECT wal_fpi, wal_write
FROM pg_stat_wal;
-- wal_fpi jumps after CHECKPOINT
\end{lstlisting}
  \end{columns}
...
% -----------------------------------------------------------------------------
\begin{frame}[fragile]{A3. Tuple Structure}
