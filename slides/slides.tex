...
\begin{frame}{Workshop Structure}
  \begin{columns}[T]
    ...
    \begin{itemize}\small
      ...
      \item {\color{gray}WAL Writer \& \texttt{synchronous\_commit} \textit{(optional)}}
    \end{itemize}

    \column{0.48\textwidth}
    \textbf{\faDatabase\enspace Module 2 --- MVCC \& Concurrency}
    \begin{itemize}\small
      \item Isolation anomalies \& the MVCC model
      \item Snapshot isolation: xmin, xmax, visibility
      \item Lost updates \& \texttt{SELECT FOR UPDATE}
      \item VACUUM \& bloat
    \end{itemize}
  \end{columns}
...
% =============================================================================
\section{WAL Performance Tuning}
% =============================================================================

% -----------------------------------------------------------------------------
% Slide: Optional section divider
% -----------------------------------------------------------------------------
\begin{frame}{WAL Performance Tuning}

  \begin{center}
    {\large \textit{Optional --- cover if time allows}}
  \end{center}

  \vspace{0.6em}
  \begin{itemize}
    \item \textbf{The WAL Writer \& group commit} --- how PostgreSQL
          amortises \texttt{fsync()} cost across concurrent transactions
    \item \textbf{\texttt{synchronous\_commit}} --- the per-transaction
          knob for trading a small durability window for throughput
  \end{itemize}
...
\begin{frame}{Module 1: Hands-On Exercises}
  \begin{columns}[T]
    ...
    \begin{itemize}\small
      ...
      \item Force a checkpoint; watch \texttt{wal\_fpi} spike
    \end{itemize}
    ...
    \column{0.48\textwidth}
    \textbf{Exercise 1.2 --- \texttt{synchronous\_commit} \textit{(optional)}}
    \begin{itemize}\small
...
% -----------------------------------------------------------------------------
% Slide: Isolation motivation — bank transfer anomaly
% -----------------------------------------------------------------------------
\begin{frame}[fragile]{The Isolation Problem}
...
% -----------------------------------------------------------------------------
% Slide: Solution approach 1 — locking
% -----------------------------------------------------------------------------
\begin{frame}[fragile]{Approach 1: Locking}
...
\begin{lstlisting}[style=ascii]
  ...
  UPDATE Arun   1000 -> 700
  UPDATE Babita  500 -> 800
  (holds locks on both rows)           BEGIN
                                       SELECT Arun ... BLOCKED
                                           |
                                           | waiting for T1
                                           |
...
{\small Correct --- but T2 sat idle while T1 worked.
At high concurrency, that wait queue becomes the bottleneck.}

\end{frame}

% -----------------------------------------------------------------------------
% Slide: Solution approach 2 — MVCC
% -----------------------------------------------------------------------------
\begin{frame}[fragile]{Approach 2: MVCC}

\begin{lstlisting}[style=ascii]
  T1: transfer Arun -> Babita (300)    T2: audit
  ----------------------------------   -------------------------
  BEGIN
  UPDATE Arun   1000 -> 700            BEGIN  <-- snapshot here
  UPDATE Babita  500 -> 800
                                       SELECT Arun   --> 1000 (*)
  COMMIT
                                       SELECT Babita -->  500 (*)
...
{\small How does PostgreSQL know which version to show T2? \textrightarrow\ next.}
...
% -----------------------------------------------------------------------------
% Slide: MVCC guarantee
% -----------------------------------------------------------------------------
\begin{frame}{MVCC: The Guarantee}

  \begin{block}{}
    \textbf{Readers never block writers.
    Writers never block readers.}\\[0.4em]
...
{\small \texttt{xmin}: txid that created this version \quad
\texttt{xmax}: txid that superseded it (\texttt{0} = live) \quad
\texttt{ctid}: physical location of the live version.}
...
% -----------------------------------------------------------------------------
\begin{frame}[fragile]{Transaction Snapshots: The Visibility Fence}
...
% -----------------------------------------------------------------------------
% Slide: Transaction Snapshots — READ COMMITTED vs REPEATABLE READ
% -----------------------------------------------------------------------------
\begin{frame}[fragile]{Transaction Snapshots: When is Yours Taken?}

\begin{lstlisting}[style=ascii]
 time   T1: transfer (txid 200)   T2: READ COMMITTED    T3: REPEATABLE READ
 ----   ----------------------    ------------------    -------------------
  t1    BEGIN
        UPDATE Arun 1000->700     BEGIN                 BEGIN
  t2                              SELECT Arun           SELECT Arun
...
{\small \texttt{READ COMMITTED}: fresh snapshot per statement --- sees T1's commit at t4.\\
\texttt{REPEATABLE READ}: snapshot fixed at \texttt{BEGIN} --- consistent view throughout.}
...
t_xmin COMMITTED?
  ...
  t_xmax COMMITTED,
    not active in snapshot     --> invisible  (deleted and visible)
\end{lstlisting}

  {\small Dirty reads are structurally impossible: a tuple written by an
  in-progress transaction is always invisible to other sessions.}
...
% -----------------------------------------------------------------------------
% Slide: Lost Update — Arun/Babita/Chirag example
% -----------------------------------------------------------------------------
...
\begin{lstlisting}[style=ascii]
  Initial:  Arun=1000  Babita=500  Chirag=1500  (total = 3000)
  ---------------------------------------------------------------
  T1: Arun -> Babita (300)          T2: Chirag -> Arun (200)
  --------------------------------  -------------------------------
  BEGIN
  SELECT Arun --> 1000
                                    BEGIN
...
% -----------------------------------------------------------------------------
% Slide: SELECT FOR UPDATE — ASCII art (Arun example)
% -----------------------------------------------------------------------------
...
\begin{lstlisting}[style=ascii]
  ...
  --------------------------------  ------------------------------
  BEGIN
  SELECT Arun FOR UPDATE            BEGIN
  --> ExclusiveLock acquired        SELECT Arun FOR UPDATE
  --> Arun = 1000                   --> BLOCKS  (lock held by T1)
  new_balance = 1000 - 300 = 700         |
  UPDATE Arun SET balance = 700          |
...
% -----------------------------------------------------------------------------
% Slide: SELECT FOR UPDATE — mechanics
% -----------------------------------------------------------------------------
\begin{frame}{\texttt{SELECT FOR UPDATE}: How It Works}

  \begin{block}{Row-level ExclusiveLock}
    \texttt{SELECT \ldots\ FOR UPDATE} acquires a row-level
    \textbf{ExclusiveLock} \emph{before} returning the value to the
    application.  A concurrent session attempting the same lock
    blocks until the holder commits or rolls back.
  \end{block}

  \vspace{0.6em}

  \begin{block}{REPEATABLE READ: first-updater-wins}
    Under \texttt{REPEATABLE READ}, the waiting session does
    \emph{not} silently overwrite.  On unblock it detects the
    concurrent update and aborts with a serialization error.
    The application must catch and retry.
  \end{block}

  \vspace{0.4em}
  {\small Inspect live locks with \texttt{pg\_locks} and
  \texttt{pg\_blocking\_pids()} --- Exercise 2.4.}
...
\begin{frame}{Key Takeaways: WAL \& Durability}
...
\begin{frame}{Key Takeaways: MVCC \& Concurrency}

  \begin{block}{MVCC \& Concurrency}
    \begin{itemize}\small
      \item MVCC prevents isolation anomalies without locking ---
